...
export default function PublicWorkspaceShelf({ contextUserId }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-6 pt-4">
      <div className="mb-4 border-t border-[#1F1F1F] pt-4">
        <div className="flex items-center justify-between">
          <h2 className="font-['Sora'] text-[30px] font-bold text-[#F5F0E8]">Public Vaults</h2>
          <p className="text-[12px] text-[#5C5248]">Discover community vaults</p>
        </div>
      </div>
      ...
      {publicWorkspacesLoading ? (
        <div className="space-y-2">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="h-[78px] w-full animate-pulse rounded-[14px] border border-[#1F1F1F] bg-[#0D0D0D]" />
          ))}
        </div>
        ...
        <motion.div
          ...
          className="rounded-[14px] border border-[#1F1F1F] bg-[#0D0D0D] p-6 text-center text-sm text-[#A09080]"
        >
